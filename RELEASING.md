...
Releases are created from `main` by semantic-release in `.github/workflows/test.yml`. The release job waits for the complete Node.js and operating-system test matrix, publishes the package to npm, creates the Git tag, and creates the GitHub release.
...
Enter only `test.yml` for the workflow filename, not `.github/workflows/test.yml`.
...
- The repository workflow must remain at `.github/workflows/test.yml`. Renaming it requires updating the npm trusted publisher first.
...
3. The npm package version shows a provenance attestation linked to `vercel/fun` and `.github/workflows/test.yml`.
